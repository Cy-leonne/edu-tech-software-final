...
const { testDatabaseFallbackEnabled } = require('../utils/securityEnv.js');
const {
...
const getFallbackAdmin = (identifier) => {
    ...
    // It is only consulted when explicitly enabled outside production.
...
const getFallbackAdmins = (query = {}) => {
    if (!testDatabaseFallbackEnabled()) return [];
    try {
        const { testDB } = require('../testdb');
...
const getAccountants = async (req, res) => {
    try {
        ...
        let currentAdmin;
        if (mongoose.connection.readyState !== 1) {
            currentAdmin = testDatabaseFallbackEnabled() ? findAdmin(currentAdminId) : null;
        } else {
            currentAdmin = await Admin.findById(currentAdminId);
        }
...
const requestAdminPasswordReset = async (req, res) => {
    try {
        ...
        const admin = await Admin.findOne(buildCaseInsensitiveEmailQuery(email));

        // Generic response for unknown accounts (prevents user enumeration and
        // removes the previous "Admin not found" 404 disclosure).
        if (!admin) {
        ...
        const resetUrl = buildResetUrl(rolePath, token);
        const emailResult = await sendResetPasswordLink(admin.email, admin.name, resetUrl);
        if (!emailResult?.success) {
            console.error('[PASSWORD RESET] Delivery failed for admin/HR/accountant reset request');
        }

        // The delivery result (and any provider message id) is intentionally not
        // returned to the caller.
        return res.send({ message: GENERIC_RESET_REQUEST_MESSAGE });
    } catch (error) {
        console.error('Admin password reset request failed:', error.message || error);
        return res.status(500).json({ message: 'Unable to process the request right now. Please try again later.' });
    }
};
...
const resetAdminPassword = async (req, res) => {
    try {
        ...
        const admin = await findUserByResetToken(Admin, token);
        ...
        // A completed reset also clears any active lockout.
        admin.failedLoginAttempts = 0;
        admin.lockoutUntil = null;
        await admin.save();
        ...
        return res.status(500).json({ message: 'Password reset failed. Please request a new reset link.' });
    }
};
...
module.exports = {
    ...
    sendAdmin2FACode,
    verifyAdmin2FACode,
    // Exported for unit testing / reuse (school scoping of dashboard stats)

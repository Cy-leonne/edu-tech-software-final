...
const searchAssignments = async (req, res) => {
    try {
        ...
        const conditions = [];
        // SECURITY: user supplied text is escaped before it is used as a regex
        // pattern (regular-expression injection / ReDoS).
        const safeTitle = sanitizeSearchTerm(title);
        const safeTeacherEmail = sanitizeSearchTerm(teacherEmail);
        const safeQuery = sanitizeSearchTerm(query);
        if (safeTitle) {
            conditions.push({ title: { $regex: safeTitle, $options: 'i' } });
        }
        if (safeTeacherEmail) {
            // join via teacher email
            const teachers = await Teacher.find({ email: { $regex: safeTeacherEmail, $options: 'i' } }).select('_id');
            const teacherIds = teachers.map(t => t._id);
            if (teacherIds.length > 0) conditions.push({ teacher: { $in: teacherIds } });
        }
        if (safeQuery) {
            conditions.push({ title: { $regex: safeQuery, $options: 'i' } });
            conditions.push({ description: { $regex: safeQuery, $options: 'i' } });
        }

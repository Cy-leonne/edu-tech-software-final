...
const getAllSchools = async (req, res) => {
    try {
        ...
        // SECURITY: escape user supplied search text before using it as a regex.
...
const getSystemLogs = async (req, res) => {
    try {
        ...
        if (!requester && testDatabaseFallbackEnabled()) {
            // SECURITY: the in-memory demo database is only consulted when it is
            // explicitly enabled for non-production environments.
...
const getAllAdmins = async (req, res) => {
    try {
        ...
        // SECURITY: escape user supplied search text before using it as a regex.
        const safeAdminSearch = sanitizeSearchTerm(search);
        if (safeAdminSearch) {
            filter.$or = [
                { name: { $regex: safeAdminSearch, $options: 'i' } },
                { email: { $regex: safeAdminSearch, $options: 'i' } },
                { schoolName: { $regex: safeAdminSearch, $options: 'i' } }
            ];
        }

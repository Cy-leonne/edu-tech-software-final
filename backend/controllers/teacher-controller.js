    ...
    try {
        ...
        if (req.query.email) {
            ...
            if (emailSearch.length > 0) {
                const safeEmailSearch = sanitizeSearchTerm(emailSearch);
                if (safeEmailSearch) {
                    query.email = { $regex: safeEmailSearch, $options: 'i' };
                }
            }
        }
...
const searchTeacher = async (req, res) => {
    try {
        ...
        const conditions = [];
        if (email) {
            conditions.push({ email: { $regex: `^${sanitizeSearchTerm(email)}`, $options: 'i' } });
        }
        if (query) {
            const safeQuery = sanitizeSearchTerm(query);
            if (safeQuery) {
                conditions.push({ name: { $regex: safeQuery, $options: 'i' } });
                conditions.push({ email: { $regex: safeQuery, $options: 'i' } });
            }
        }
...
const requestTeacherPasswordReset = async (req, res) => {
    try {
        ...
        // Generic response: never disclose whether the account exists.
        if (!teacher) {
            return res.send({ message: GENERIC_RESET_REQUEST_MESSAGE });
        }
        ...
        const resetUrl = buildResetUrl('Teacher', token);
        const emailResult = await sendResetPasswordLink(teacher.email, teacher.name, resetUrl);
        if (!emailResult?.success) {
            console.error('[PASSWORD RESET] Delivery failed for teacher reset request');
        }

        return res.send({ message: GENERIC_RESET_REQUEST_MESSAGE });
    } catch (error) {
        console.error('Teacher password reset request error:', error.message || error);
...
const resetTeacherPassword = async (req, res) => {
    try {
        ...
        const teacher = await findUserByResetToken(Teacher, token);
        if (!teacher) {
        ...
        clearResetToken(teacher);

        await teacher.save();
        return res.send({ message: 'Password reset successfully' });
    } catch (error) {
        console.error('Teacher password reset error:', error.message || error);
...
const resetTeacherPasswordByAdmin = async (req, res) => {
    try {
        ...
        if (!passwordCheck.valid) return res.status(400).send({ message: passwordCheck.error });
        ...
        console.error('Teacher password reset by admin failed:', err.message || err);

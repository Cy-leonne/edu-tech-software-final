...
// Capture the raw JSON body for the M-Pesa callback route so that HMAC
// signature verification (x-mpesa-signature) can be computed against the
// exact bytes Safaricom/a relay sent.
app.use(express.json({
    limit: '10mb',
    verify: (req, res, buf) => {
        if (req.originalUrl && req.originalUrl.startsWith('/Payment/MpesaCallback')) {

...
// Payment provider webhook limiter: 120 callbacks per 5 minutes per IP.
// Callbacks are machine to machine, so a higher, still bounded, limit applies.
const paymentWebhookLimiter = rateLimit({
    windowMs: 5 * 60 * 1000,
    max: 120,
    message: 'Too many payment callbacks received.',
    standardHeaders: true,
    legacyHeaders: false,
    keyGenerator: (req, res) => ipKeyGenerator(req),
});

// Parent portal limiter: 60 requests per 15 minutes per IP. Slows down
// email/admission-number enumeration on the parent portal.

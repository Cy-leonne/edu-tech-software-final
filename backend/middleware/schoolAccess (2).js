...
const enforceSubscriptionStatus = async (schoolId) => {
    ...
    const schoolQuery = School.findOne({
        $or: [{ _id: schoolId }, { schoolAdmin: schoolId }]
    });
    // Some tests substitute models with promise-returning mocks; chain .select
    // only when a real mongoose Query is available.
    const school = typeof schoolQuery?.select === 'function'
        ? await schoolQuery.select('_id schoolName status subscriptionStatus currentSubscription statusChangeReason')
        : await schoolQuery;

    if (!school) return null;

    // Subscription bookkeeping requires a live connection; degrade to the
    // persisted school state when the database is unavailable.
    if (mongoose.connection.readyState !== 1) return school;
...
/**
 * Default response for cross-school attempts.
 */
const denyCrossSchool = (res) => {
    res.status(403).send({ message: 'Access denied: resource belongs to another school' });
    return false;
};

/**
 * Guard helper shared by verifySchoolId / verifyEntityBelongsToAdminSchool.
 * Returns the request user object or null after having written an error
 * response (401 unauthenticated, 403 subscription-suspended).
 */
const authenticateRequestForSchool = async (req, res) => {
...
const verifySchoolId = async (req, res, schoolId) => {
    const requestUser = await authenticateRequestForSchool(req, res);
    if (!requestUser) return false;

    // SuperAdministrator operates across all schools.
    if (requestUser.type === 'admin' && isSuperAdmin(requestUser.user)) {
        return true;
    }

    // Legacy clients occasionally serialise an ObjectId placeholder instead of
    // the actual id; treat it as "no specific school id requested".
    const requested = schoolId && schoolId.toString();
    if (!requested || requested === '[object Object]' || requested === 'undefined' || requested === 'null') {
        return true;
    }

    // Every other role is strictly scoped to its own school.
    if (!requestUser.schoolId) {
        return denyCrossSchool(res);
    }
    if (requestUser.schoolId !== requested) {
        return denyCrossSchool(res);
    }
...
const verifyEntityBelongsToAdminSchool = async (req, res, entity, schoolField = 'school') => {
    ...
    // If the entity has no school information there is nothing to compare —
    // keep the historical permissive behaviour for legacy records.
    if (!entitySchoolId) {
        return true;
    }

    if (!requestUser.schoolId || requestUser.schoolId !== entitySchoolId) {
        return denyCrossSchool(res);
    }

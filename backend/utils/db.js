...
const { redactConnectionString } = require('./securityEnv');
...
const connectToMongo = async (uri, options = {}) => {
    ...
    const connect = async () => {
        ...
        try {
            ...
            // Never print credentials that may be embedded in the URI.
            console.log(`Connected to MongoDB (${redactConnectionString(uri)})`);
            await ensureAdminSchoolIndexes();
            return true;
        } catch (err) {
            console.error(`MongoDB connection attempt ${attempt} failed:`, redactConnectionString(err.message || err));
